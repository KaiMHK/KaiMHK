...
vec4 applyTextureFunction() {
	...
	if ( InFshape(wrappedTexCoords) ) {		
		//float x = sin*currentTime;
		return vec4( sin(currentTime*3.14)*1, sin(currentTime*3.14)*0.8, 0, 1 );                // Black color inside the "F"
...
bool InFshape( vec2 pos ) {
	...
	*/
	...
	float R = 0.49;
	float r = 0.4;
	if(((pos.x-0.5)*(pos.x-0.5)+(pos.y-0.5)*(pos.y-0.5))<R*R&&(((pos.x-0.7)*(pos.x-0.7)+(pos.y-0.5)*(pos.y-0.5))>r*r)) return true;
	else return false;
